...
object DWReleaseClick {
  ...
  def handleReleaseJob(spark:SparkSession,appName:String,bdp_day:String): Unit ={
    try{
      ...
      // 获取当天日志字段数据
      val clickColumns: ArrayBuffer[String] = DWReleaseClickHelper.selectDWReleaseColumns()
      ...
      // 填入条件
      //这个DF就是读取ods表数据之后的
      val clickReleaseDF: DataFrame = SparkHelper
        .readTableData(spark,ReleaseConstant.ODS_RELEASE_SESSION,clickColumns)
        // 查询条件
  ...
  def main(args: Array[String]): Unit = {
    ...
    val appName :String = "dw_click_customer_job"
