...
object DM_RELEASE_CLICK_CUBE {
  ...
  def handleReleaseJob(spark:SparkSession,appName:String,bdp_day:String): Unit ={
    try{
      ...
      // 获取曝光主题字段数据
      val cusomerColumns: ArrayBuffer[String] = DM_RELEASE_CLICK_CUBE_Helper.selectDWReleaseColumns()
      ...
      val customerReleaseDF: DataFrame = SparkHelper
        .readTableData(spark,ReleaseConstant.DW_RELEASE_CLICK,cusomerColumns)
        .alias("a")
        // 查询条件
      ...
      //获取需要join的表的字段
      val custormDF: Dataset[Row] = SparkHelper
        .readTableData(spark,ReleaseConstant.DW_RELEASE_CUSTOMER,DM_Customer_Sources_Helper.selectDMCustomerSourceColumns())
        .alias("b")

      val l: Long = customerReleaseDF.count()

      val showDF: DataFrame = customerReleaseDF.join ( custormDF, "release_session" )
        .groupBy ( "a.sources", "a.channels", "age_range","aid")
        .agg ( count ( "*" ))
      ...
      // 目标用户
      SparkHelper.writeTableData(showDF,ReleaseConstant.DM_CLICK_CUBE,saveMode)
