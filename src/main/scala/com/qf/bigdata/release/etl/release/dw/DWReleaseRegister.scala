...
object DWReleaseRegister {
  ...
  def handleReleaseJob(spark:SparkSession,appName:String,bdp_day:String): Unit ={
    try{
      ...
      // 获取当天日志字段数据
      val registerColumns: ArrayBuffer[String] = DWReleaseRegisterHelper.selectDWReleaseColumns()
      ...
      val registerReleaseDF: DataFrame = SparkHelper
        .readTableData(spark,ReleaseConstant.ODS_RELEASE_SESSION,registerColumns)
        // 查询条件
