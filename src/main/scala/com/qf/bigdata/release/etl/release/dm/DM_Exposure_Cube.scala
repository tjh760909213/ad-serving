...
import com.qf.bigdata.release.etl.release.dw.{DWReleaseCustomer, DWReleaseExposureHelper}
...
object DM_Exposure_Cube {
  ...
  def handleReleaseJob(spark:SparkSession,appName:String,bdp_day:String): Unit ={
    try{
      ...
      // 获取曝光主题字段数据
      val cusomerColumns: ArrayBuffer[String] = DMReleaseExposureHeelper.selectDWReleaseColumns()
...
//            lit(ReleaseStatusEnum.SHOW.getCode)
      ...
      val customerReleaseDF: DataFrame = SparkHelper
        .readTableData(spark,ReleaseConstant.DW_RELEASE_EXPOSURE,cusomerColumns)
        .alias("a")
        // 查询条件
      ...
      //获取需要join的表的字段
      val custormDF: Dataset[Row] = SparkHelper.readTableData(spark,ReleaseConstant.DW_RELEASE_CUSTOMER,DM_Customer_Sources_Helper.selectDMCustomerSourceColumns()).alias("b")

      val l: Long = customerReleaseDF.count()

      val showDF: DataFrame = customerReleaseDF.join ( custormDF, "release_session" )
        .groupBy ( "a.sources", "a.channels", "a.device_type", "age_range", "gender", "area_code" )
        .agg ( count ( "*" ),
          count ( "*" ) / l)
      ...
      println("查询结束======================结果显示")
      showDF.show(10,false)
      // 目标用户
      SparkHelper.writeTableData(showDF,ReleaseConstant.DM_EXPOSURE_CUBE,saveMode)
