...
import com.qf.bigdata.release.etl.release.dw.{DWReleaseColumnsHelper, DWReleaseCustomer, DWReleaseExposureHelper}
...
object DM_Exposure_Sources {
  ...
  def handleReleaseJob(spark:SparkSession,appName:String,bdp_day:String): Unit ={
    try{
      ...
      // 获取曝光主题字段数据
      val cusomerColumns: ArrayBuffer[String] = DWReleaseExposureHelper.selectDWReleaseColumns()
...
//            lit(ReleaseStatusEnum.SHOW.getCode)
      ...
      val customerReleaseDF: DataFrame = SparkHelper
        .readTableData(spark,ReleaseConstant.DW_RELEASE_EXPOSURE,DM_Customer_Sources_Helper.selectDMCustomerSourceColumns())
        // 查询条件
      ...
      val sum: Long = customerReleaseDF.count()

      val ES: DataFrame = customerReleaseDF.groupBy("sources","channels", "device_type")
        .agg(count("*") as ("count"),count("*")/sum,max("bdp_day"))

      println("查询结束======================结果显示")
      ES.show(10,false)
      // 目标用户
      SparkHelper.writeTableData(ES,ReleaseConstant.DM_EXPOSURE_SOURCES,saveMode)
